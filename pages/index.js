    ...
    <div>
      ...
      >
        <div className="w-full flex flex-col gap-10 ">
          ...
          <button className="bg-rose-500 hover:-translate-y-1.5 duration-150 ease-in hover:bg-rose-600 p-4 shadow-lg shadow-rose-500/40 font-bold text-white text-xl rounded-full max-w-[200px] w-full">
            Call Now
          </button>
        </div>
      ...
      <div className=" bg-white py-20 md:py-32 px-5   w-full">
        <div className="max-w-7xl flex md:flex-row flex-col gap-28 md:gap-10 sm:gap-20 mx-auto items-center ">
          <div className="flex flex-col gap-7 text-left w-full md:w-5/12">
            ...
            <Paragraph extra="text-zinc-700">
              A conversation with astrologers will give you a new outlook
              towards life. Online astrology consultation gives you a chance to
              identify your strengths and weakness without revealing your
              identity.
            </Paragraph>
            <button className="hover:bg-green-600 bg-green-500 text-white mt-2 shadow-lg shadow-green-400/30  rounded-[60px] p-4 text-xl font-semibold max-w-[230px] hover:-translate-y-2 transition duration-150 ease-in justify-items-center">
              Call Now
            </button>
          </div>
          <div className="flex  justify-center gap-5 w-full md:w-7/12	">
            <div className="grid grid-cols-2 grid-rows-2 max-w-lg w-full gap-10 circle">
              <div className="flex font-bold bg-white shadow-xl rounded-[30px] text-zinc-700  flex-col py-10 px-7 w-full justify-center text-center gap-3 text-3xl md:text-4xl">
                1000+
                <span className="text-lg md:text-xl font-normal leading-6 text-zinc-700">
                  Total Astrologers
                </span>
              </div>
              <div className="row-span-2  flex justify-center">
                <div className="flex font-bold  w-full my-auto  bg-white  shadow-xl rounded-[30px] text-zinc-700  flex-col md:py-10  py-8 px-7 justify-center text-center gap-3 text-3xl leading-7 md:text-4xl">
                  5000+ Min
              ...
              <div className="flex font-bold bg-white  shadow-xl rounded-[30px] text-zinc-700  flex-col py-10 px-7 justify-center text-center gap-3 text-3xl md:text-4xl">
                100+
                <span className="text-lg leading-6 md:text-xl font-normal">
                  Total Customers
                </span>
              </div>
            </div>
      ...
      <Section section="bg-gray-50" py={"py-28"} extra="flex flex-col gap-10">
        <div className="flex flex-col gap-7 max-w-3xl mx-auto text-center">
          ...
          <div>
            <Paragraph>
              100% secure and confidential. Connect with Astrologer and get
              complete Astrology predictions. No data or information breach.
            </Paragraph>
          </div>
        </div>
        <LazyLoadComponent beforeLoad={() => setstate(true)}>
          <div className=" max-w-xl mx-auto mt-10 w-full">
            <ul className="flex text-xl md:text-2xl  flex-col gap-24 w-full">
              <li
                className={`flex ${
                  true
                    ? "before:animate-[wiggle_1.5s_ease-in-out_forwards]"
                    : "before:h-[0px]"
                } items-center z-[2] relative before:absolute before:top-full before:z-[1] before:left-1/2  before:w-[4px] before:bg-orange-500 font-semibold gap-7`}
              >
                <span className="w-full"></span>
                <div className="p-5 rounded-full bg-orange-100 w-full max-w-max">
                  <img
                    className="w-12 h-12"
                    src="/icon/user.svg"
                    alt="signup"
              ...
              <li
                className={`${
                  state
                    ? "before:animate-[wiggle_1.5s_ease-in-out_1.5s_forwards]"
                    : "before:h-[0px]"
                } flex relative z-[2] before:absolute before:top-full before:z-[1] before:left-1/2  before:w-[4px]  before:bg-lime-500 items-center  font-semibold gap-7`}
              >
                <span className="w-full "> Recharge Wallet</span>
                <div className="p-5 rounded-full bg-green-100 w-full max-w-max">
                  <img
                    className="w-12 h-12"
                    src="/icon/wallet.svg"
                    alt="signup"
              ...
              <li
                className={`${
                  state
                    ? "before:animate-[wiggle_1.5s_ease-in-out_3s_forwards]"
                    : "before:h-[0]"
                } flex relative z-[2] before:absolute before:top-full before:z-[1] before:left-1/2  before:w-[4px] before:bg-sky-500 items-center  font-semibold gap-7`}
              >
                <span className="w-full"></span>
                <div className="p-5 rounded-full bg-sky-100 w-full max-w-max">
                  <img
                    className="w-12 h-12"
        ...
        <div className="w-full">
          <div className=" flex overflow-x-auto w-full py-10">
            {review.map((item, i) => (
              <div
                key={i}
                className="flex relative w-full  min-w-[350px] cursor-default mr-10 flex-col hover:shadow-xl hover:shadow-rose-200/40 gap-6 py-12 px-10 border border-rose-200 bg-white rounded-xl"
              >
