...
export default function Mobilesidemenu(props) {
  ...
  return (
    <div className="fixed z-20 bg-white flex-col lg:hidden h-full  w-[300px] pt-10  gap-10 flex">
      <button
        onClick={() => props.passclose(true)}
        className="absolute top-3 right-3 p-1 lg:hidden block rounded-full bg-red-400"
      >
        <svg
      ...
      <ul className="w-full flex flex-col gap-3 md:gap-5">
        <li
          ...
          className={`${
            props.active === "home"
              ? " border-r-4  border-red-500 bg-red-50  bg-opacity-50 text-red-500 "
              : "text-zinc-500"
          } py-3  px-6 relative  w-full cursor-pointer font-semibold  text-lg flex items-center gap-5`}
        >
          <svg
            id="Layer_1"
            version="1.1"
        ...
        <li
          onClick={() => {
            router.push("/astrologer-admin/call-history");
            props.passclose(true);
          }}
          className={`${
            props.active === "call-history"
              ? "border-r-4  border-red-500 bg-red-50  bg-opacity-50 text-red-500 "
              : "text-zinc-500"
          } py-3 px-6 w-full cursor-pointer font-semibold  text-lg flex items-center gap-5`}
        >
          <svg
        ...
        <li
          onClick={() => {
            router.push("/astrologer-admin/profile-view");
            props.passclose(true);
          }}
          className={`${
            props.active === "profile"
              ? "border-r-4  border-red-500 bg-red-50 bg-opacity-50 text-red-500"
              : "text-zinc-500"
          }  py-3 px-6 w-full  font-semibold cursor-pointer text-lg flex items-center gap-5`}
        >
          <svg
            version="1.1"
            viewBox="0 0 50 50"
      ...
      <button onClick={()=> {localStorage.removeItem("astrologerSignup");router.reload()}} className="flex md:hidden   mt-auto w-full hover:bg-zinc-800  py-3 px-5  font-semibold text-zinc-600 hover:text-zinc-100 items-center justify-between">
        Logout
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-5 w-5"
